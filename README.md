...
Everything except the BigQuery data and trained models lives in this repo. The 9 figures in `results/figures/` were generated by the chained pipeline above with no manual editing. Re-running on a fresh machine should reproduce all results modulo small numerical noise (BERTopic uses random initialization with `random_state=42`).
